...
function isValidKey_(provided, expected) {
...
function getConfig_() {
  ...
  return {
    ...
    timezone:         props.getProperty('timezone')         || CONFIG_DEFAULTS.timezone,
...
function doGet(e) {
  ...
  const readKey = getSecret_('READ_KEY');
  const writeKey = getSecret_('WRITE_KEY');

  // getConfig è accessibile con entrambe le chiavi (web app usa WRITE_KEY, notifier usa READ_KEY)
  if (action === 'getConfig') {
    if (!isValidKey_(key, readKey) && !isValidKey_(key, writeKey)) return unauthorized_();
    return ContentService
  ...
  if (!isValidKey_(key, readKey)) return unauthorized_();
...
function doPost(e) {
  ...
  if (!isValidKey_(key, getSecret_('WRITE_KEY'))) return unauthorized_();

  if (action === 'setConfig') {
    const props   = PropertiesService.getScriptProperties();
    const allowed = ['paused', 'timezone', 'window_start', 'window_end', 'min_per_day', 'max_per_day', 'min_gap_minutes'];
    allowed.forEach(k => {
